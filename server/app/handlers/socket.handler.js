...
const queryHandler = require('../services/user.service');
		...
		this.io.on('connection', (socket) => {
			...
			socket.on(`add-message`, async (data) => {
				...
				if (data.message === '') {
					...
					try {
						const [toUser, messageResult] = await Promise.all([
							queryHandler.getUserInfo({
						...
						let toUserSocketId = toUser[0]['socketId'];
						this.io.to(toUserSocketId).emit(`add-message-response`, data);
					} catch (error) {
						this.io.to(socket.id).emit(`add-message-response`, {
							error: true,
